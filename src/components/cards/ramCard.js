...
function RamCard({ singleProduct }) {
  ...
  const handleAddComponents = async(singleComponent) => {
    ...
    if(existingComponents){
      await Swal.fire({
         icon: 'error',
         title: 'Oh Dear',
         text: 'You have already added Ram to your build',
       })
    ...
    <div className="md:flex items-start justify-center py-12 2xl:px-20 md:px-6 px-4 mt-[6rem]">
      ...
      <div className="xl:w-2/5 md:w-1/2 lg:ml-8 md:ml-6 md:mt-0 mt-6">
        ...
        <div>
          ...
          <p className="text-base leading-4 mt-4 text-gray-600 dark:text-gray-300">
            Type: {singleProduct.KeyFeatures.Type}
          </p>
        </div>
        <div>
        ...
        <div>
          <div className="border-b py-4 border-gray-200">
            ...
            >
              <p>Voltage : {singleProduct.KeyFeatures.Voltage}</p>
              <p>Cas Latency : {singleProduct.KeyFeatures.CasLatency}</p>
              <p>Speed : {singleProduct.KeyFeatures.Speed}</p>
            </div>
          </div>
        </div>
      </div>
